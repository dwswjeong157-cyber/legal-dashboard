...
st.set_page_config(page_title="대웅 법무 - 파일 업로드 분석", layout="wide")
...
with col_title:
    ...
    st.markdown(
        ...
        f"File Upload Mode | 파일 업로드 즉석 분석"
        f"</p>",
...
if not email_configured:
    ...
    receiver_input = col_f1.text_input("메일 주소 입력", value="@daewoong.co.kr", key="upload_email")

    if col_f2.button("🚀 리포트 발송", use_container_width=True, key="upload_send"):
        with st.spinner("리포트 생성 중..."):
            try:
                ...
                msg['Subject'] = f"[대웅법무] 업무 현황 리포트 - 업로드 분석 ({now_kst.strftime('%y.%m.%d')})"
                msg['From'] = S_MAIL
                ...
                html_mail = (
                    ...
                    f'<p style="margin:5px 0 0; font-size:13px; opacity:0.9;">파일 업로드 분석 결과</p>'
                    f'</div>'
